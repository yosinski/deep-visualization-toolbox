 ...
 * If the backprop pane in the lower left is just gray, it's because backprop and deconv are producing all zeros, in which case you probably just need to add "force_backward: true" to your network prototxt. By default, Caffe won't compute derivatives at the data layer, because they're not needed to update parameters.
